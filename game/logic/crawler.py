...
class Crawler(BaseLogic):
    ...
    def check_density(self, diamond: GameObject, bot_pos: Position) -> float:
    ...
    def generate_best_density(
        ...
        for diamond in diamonds:
            d = self.check_density(diamond, bot_position)
            if d > best_direct[0]:
    ...
    def next_move(self, board_bot: GameObject, board: Board) -> tuple:
        ...
        prima = list_teleporters[0]
        sembiring = list_teleporters[1]
        targetTeleporter = prima
        exitTeleporter = sembiring

        # -- cari jarak manhattan dan simpan ke variable -- #
        dm = self.needed_steps(prima.position, current_position)
        dp = self.needed_steps(sembiring.position, current_position)

        # -- ubah teleporter terdekat dan terjauh sesuai posisi saat ini -- #
        if dp < dm:
            targetTeleporter = sembiring
            exitTeleporter = prima
